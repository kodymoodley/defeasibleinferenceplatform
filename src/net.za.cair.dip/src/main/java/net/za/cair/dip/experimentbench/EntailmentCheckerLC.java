...
public class EntailmentCheckerLC {
	...
	public static void main(String[] args) throws OWLOntologyCreationException, OWLOntologyStorageException, IOException, ExecutionException, ExplanationException, TimeoutException{
		...
		FileWriter avgQueryTimeRCW = new FileWriter("test2/Avg_query_time_LC.txt");
		PrintWriter avgQueryTimeRCPW = new PrintWriter(avgQueryTimeRCW);
			
		FileWriter avgEntailmentChecksRCW = new FileWriter("test2/Avg_entailment_checks_LC.txt");
		PrintWriter avgEntailmentChecksRCPW = new PrintWriter(avgEntailmentChecksRCW);
		
		FileWriter totalAxiomsInCCompatRCW = new FileWriter("test2/Avg_axioms_in_ccompat_LC.txt");
		PrintWriter totalAxiomsInCCompatRCPW = new PrintWriter(totalAxiomsInCCompatRCW);
		
		FileWriter yesAnswersRCW = new FileWriter("test2/Avg_number_of_yes_answers_LC.txt");
		PrintWriter yesAnswersRCPW = new PrintWriter(yesAnswersRCW);

		/*FileWriter infiniteRankQueryW = new FileWriter("test2/Avg_cases_of_infinite_rank_query.txt");
		PrintWriter infiniteRankQueryPW = new PrintWriter(infiniteRankQueryW);
			
		FileWriter prunedRankingSizeW = new FileWriter("test2/Avg_pruned_ranking_size.txt");
		PrintWriter prunedRankingSizePW = new PrintWriter(prunedRankingSizeW);
			
		FileWriter sizeOfPrunedRankW = new FileWriter("test2/Avg_size_of_pruned_rank.txt");
		PrintWriter sizeOfPrunedRankPW = new PrintWriter(sizeOfPrunedRankW);
			
		FileWriter ranksInCCompatW = new FileWriter("test2/Avg_ranks_in_ccompat.txt");
		PrintWriter ranksInCCompatPW = new PrintWriter(ranksInCCompatW);
		
		FileWriter numberOfStrictAxiomsW = new FileWriter("test2/Number_of_strict_axioms.txt");
		PrintWriter numberOfStrictAxiomsPW = new PrintWriter(numberOfStrictAxiomsW);
		
		FileWriter numberOfDefAxiomsW = new FileWriter("test2/Number_of_defeasible_axioms.txt");
		PrintWriter numberOfDefAxiomsPW = new PrintWriter(numberOfDefAxiomsW);*/
		
		FileWriter problematicRankSizeW = new FileWriter("test2/Problematic_rank_size.txt");
		...
		for (int j = 1; j <= 35;j++){
	       ...
	       int defeasibleAxioms = 0;
			...
			while (iter.hasNext() && count < 20){
				...
				entailmentChecks += dic.entailmentChecksLC; 
				...
				problematicRankAxioms += dic.pRank.size();
