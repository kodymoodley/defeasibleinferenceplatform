...
public class DefeasibleInferenceHelperClass {
	...
	public ArrayList<Rank> getMRCCompatibleSubset(ArrayList<Rank> ranksVar, OWLClassExpression cls, Set<OWLAxiom> cbasis) throws OWLOntologyCreationException{
		ArrayList<Rank> ranks = new ArrayList<Rank>();
		for (Rank r: ranksVar){
			ArrayList<OWLAxiom> newRankAxioms = new ArrayList<OWLAxiom>();
			for (OWLAxiom a: r.getAxioms()){
		...
		while (antecedentExceptional(cls, ranks, ranking.getInfiniteRank().getAxiomsAsSet()) && rankIndex < ranking.size()){
			Rank r = ranks.get(rankIndex);
