...
public class DefeasibleAxiomsViewComponent extends AbstractActiveOntologyViewComponent {
	private static final long serialVersionUID = 2478857043399890008L;	
	private DefeasibleAxiomList dalList;
	...
	protected void initialiseOntologyView() throws Exception {									
		dalList = new DefeasibleAxiomList(getOWLEditorKit(), getOWLModelManager());
		dalList.setAxioms(getDefeasibleAxioms(getOWLModelManager().getActiveOntology()));
		setLayout(new BorderLayout());
		add(new JScrollPane(dalList));
	}
	...
	private Set<OWLAxiom> getDefeasibleAxioms(OWLOntology ontology){
		...
		return defeasibleAxioms;
	}

	@Override
	protected void updateView(OWLOntology activeOntology) throws Exception {
		dalList.setAxioms(getDefeasibleAxioms(activeOntology));
	}
}
